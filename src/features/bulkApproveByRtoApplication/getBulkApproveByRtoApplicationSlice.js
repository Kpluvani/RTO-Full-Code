...
export const fetchApproveByRtoApplications = createAsyncThunk(
  'approve-by-rto-get-application/getAll',
  async ({ page, limit, search, searchCols, include, exclude, applicationNo, workType, where }, { rejectWithValue }) => {
    try {
      const response = await axios.get('/bulk-approve-by-rto/get-application', { params: { page, limit, search, searchCols, include, exclude, applicationNo, workType, where } });
...
const getBulkSendToRtoApplicationSlice = createSlice({
  ...
  reducers: {
    ...
    extraReducers: (builder) => {
      builder
        .addCase(fetchApproveByRtoApplications.pending, (state) => {
          state.status = 'loading';
          state.loading = true;
          state.error = null;
          state.data = [];
        })
        .addCase(fetchApproveByRtoApplications.fulfilled, (state, action) => {
          state.loading = false;
          state.error = null;
        ...
        .addCase(fetchApproveByRtoApplications.rejected, (state, action) => {
          state.status = 'failed';
          state.loading = false; 
