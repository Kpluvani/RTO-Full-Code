...
import { deleteYear, updateYearStatus } from '../../../features/year/yearSlice';

export const YearTable = ({ data, total, page, limit, setPage, setLimit, loading, search = "", setSearch, handleEdit }) => {
      ...
      render: (status, record) => (
        <Switch
          ...
          onChange={async (checked) => {
            ...
            const res = await dispatch(updateYearStatus({ id: record.id, status: checked }));
            if (res.error) {
                  messageApi.open({
                    type: 'error',
      ...
      render: (_, record) => (
        <Space>
          ...
          <Popconfirm
            ...
            onConfirm={async () => {
              const res = await dispatch(deleteYear(record.id));
              if (res.error) {
                message.error(res.payload || 'Failed to delete year');
              } else {
                message.success(res.payload || 'year delete successfully');
              }
            }}
            okText="Yes"
            cancelText="No"
