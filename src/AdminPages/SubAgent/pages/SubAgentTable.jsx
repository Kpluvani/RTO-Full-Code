...
import { deleteSubAgent, updateSubAgentStatus } from '../../../features/subAgent/subAgentSlice';

export const SubAgentTable = ({ data, total, page, limit, setPage, setLimit, loading, search = "", setSearch, handleEdit }) => {
      ...
      render: (status, record) => (
        <Switch
          ...
          onChange={async (checked) => {
            ...
            const res = await dispatch(updateSubAgentStatus({ id: record.id, status: checked }));
            if (res.error) {
                  messageApi.open({
                    type: 'error',
      ...
      render: (_, record) => (
        <Space>
          ...
          <Popconfirm
            ...
            onConfirm={async () => {
              const res = await dispatch(deleteSubAgent(record.id));
              if (res.error) {
                message.error(res.payload || 'Failed to delete SubAgent');
              } else {
                message.success(res.payload || 'SubAgent delete successfully');
              }
            }}
            okText="Yes"
            cancelText="No"
