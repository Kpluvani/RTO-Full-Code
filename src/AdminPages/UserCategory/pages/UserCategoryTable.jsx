...
import { deleteUserCategory, updateUserCategoryStatus } from '../../../features/userCategory/userCategorySlice';

export const UserCategoryTable = ({ data, total, page, limit, setPage, setLimit, loading, search = "", setSearch, handleEdit }) => {
      ...
      render: (status, record) => (
        <Switch
          ...
          onChange={async (checked) => {
            ...
            const res = await dispatch(updateUserCategoryStatus({ id: record.id, status: checked }));
            if (res.error) {
                  messageApi.open({
                    type: 'error',
      ...
      render: (_, record) => (
        <Space>
          ...
          <Popconfirm
            ...
            onConfirm={async () => {
              const res = await dispatch(deleteUserCategory(record.id));
              if (res.error) {
                message.error(res.payload || 'Failed to delete User Category');
              } else {
                message.success(res.payload || 'User Category delete successfully');
              }
            }}
            okText="Yes"
            cancelText="No"
