...
import { deleteMaker, updateMakerStatus } from '../../../features/maker/makerSlice';

export const MakerTable = ({ data, total, page, limit, setPage, setLimit, loading, search = "", setSearch, handleEdit }) => {
      ...
      render: (status, record) => (
        <Switch
          ...
          onChange={async (checked) => {
            ...
            const res = await dispatch(updateMakerStatus({ id: record.id, status: checked }));
            if (res.error) {
                  messageApi.open({
                    type: 'error',
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          />
          <Popconfirm
            title="Are you sure to delete this maker?"
            onConfirm={async () => {
              const res = await dispatch(deleteMaker(record.id));
              if (res.error) {
                message.error(res.payload || 'Failed to delete maker');
              } else {
                message.success(res.payload || 'maker delete successfully');
              }
            }}
            okText="Yes"
            cancelText="No"
