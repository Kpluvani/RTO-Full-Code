...
import { deleteFuel, updateFuelStatus } from '../../../features/fuel/fuelSlice';

export const FuelTable = ({ data, total, page, limit, setPage, setLimit, loading, search = "", setSearch, handleEdit }) => {
      ...
      render: (status, record) => (
        <Switch
          ...
          onChange={async (checked) => {
            ...
            const res = await dispatch(updateFuelStatus({ id: record.id, status: checked }));
            if (res.error) {
                  messageApi.open({
                    type: 'error',
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          />
          <Popconfirm
            title="Are you sure to delete this fuel?"
            onConfirm={async () => {
              const res = await dispatch(deleteFuel(record.id));
              if (res.error) {
                message.error(res.payload || 'Failed to delete fuel');
              } else {
                message.success(res.payload || 'Fuel delete successfully');
              }
            }}
            okText="Yes"
            cancelText="No"
