...
import { deleteParty, updatePartyStatus } from '../../../features/party/partySlice';

export const PartyTable = ({ data, total, page, limit, setPage, setLimit, loading, search = "", setSearch, handleEdit }) => {
      ...
      render: (status, record) => (
        <Switch
          ...
          onChange={async (checked) => {
              ...
              const res = await dispatch(updatePartyStatus({ id: record.id, status: checked }));
              if (res.error) {
                  messageApi.open({
                    type: 'error',
      ...
      render: (_, record) => (
        <Space>
          ...
          <Popconfirm
            ...
            onConfirm={async () => {
              const res = await dispatch(deleteParty(record.id));
              if (res.error) {
                  messageApi.open({
                    type: 'error',
                    content: res.payload || 'Failed to delete Party',
                  });
              } else {
                 messageApi.open({
                    type: 'success',
                    content: res.payload || 'Party delete successfully',
                  });
              }
            }}
            okText="Yes"
