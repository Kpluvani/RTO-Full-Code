...
import { deleteMonth, updateMonthStatus } from '../../../features/month/monthSlice';

export const MonthTable = ({ data, total, page, limit, setPage, setLimit, loading, search = "", setSearch, handleEdit }) => {
      ...
      render: (status, record) => (
        <Switch
          ...
          onChange={async (checked) => {
            ...
            const res = await dispatch(updateMonthStatus({ id: record.id, status: checked }));
            if (res.error) {
                  messageApi.open({
                    type: 'error',
      ...
      render: (_, record) => (
        <Space>
          ...
          <Popconfirm
            ...
            onConfirm={async () => {
              const res = await dispatch(deleteMonth(record.id));
              if (res.error) {
                message.error(res.payload || 'Failed to delete month');
              } else {
                message.success(res.payload || 'month delete successfully');
              }
            }}
            okText="Yes"
            cancelText="No"
