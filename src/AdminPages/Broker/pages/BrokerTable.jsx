...
import { deleteBroker, updateBrokerStatus } from '../../../features/broker/brokerSlice';

export const BrokerTable = ({ data, total, page, limit, setPage, setLimit, loading, search = "", setSearch, handleEdit }) => {
      ...
      render: (status, record) => (
        <Switch
          ...
          onChange={async (checked) => {
              ...
              const res = await dispatch(updateBrokerStatus({ id: record.id, status: checked }));
              if (res.error) {
                  messageApi.open({
                    type: 'error',
      ...
      render: (_, record) => (
        <Space>
          ...
          <Popconfirm
            ...
            onConfirm={async () => {
              const res = await dispatch(deleteBroker(record.id));
              if (res.error) {
                  messageApi.open({
                    type: 'error',
                    content: res.payload || 'Failed to delete Broker',
                  });
              } else {
                 messageApi.open({
                    type: 'success',
                    content: res.payload || 'Broker delete successfully',
                  });
              }
            }}
            okText="Yes"
